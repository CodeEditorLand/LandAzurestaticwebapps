...
import {
	...
	gitignoreFileName,
	openRemoteProjectMsg,
	remoteRepositoriesId,
} from "../constants";
...
import {
	createFork,
	hasAdminAccessToRepo,
	tryGetReposGetResponseData,
} from "./gitHubUtils";
...
export async function verifyGitWorkspaceForCreation(
	...
	if (!gitWorkspaceState.repo) {
		...
		!!gitWorkspaceState.remoteRepo &&
		!gitWorkspaceState.hasAdminAccess
	) {
...
export async function remoteShortnameExists(
	...
	try {
		remoteExists = !!repo?.state.remotes.some((r) => {
