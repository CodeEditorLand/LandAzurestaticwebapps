...
async function getBuildDeployStep(
	...
	if (/Azure\/static-web-apps-deploy/.test(yamlFileContents)) {
		...
		const parsedYaml = <{ jobs?: { steps?: BuildDeployStep[] }[] }>(
			await parse(yamlFileContents, { prettyErrors: true })
		);

		for (const job of Object.values(parsedYaml.jobs || {})) {
			for (const step of Object.values(job.steps || {})) {
				if (step.id === "builddeploy" && step.with) {
					const stepIncludesAppLocation: boolean =
						stepIncludesBuildConfig(step, "app_location");
