...
export class EnvironmentTreeItem
	...
	implements IAzureResourceTreeItem
{
	...
	constructor(parent: AzExtParentTreeItem, env: StaticSiteBuildARMResource) {
		...
		this.label = this.isProduction
			? productionEnvironmentName
			: `${this.data.pullRequestTitle}`;
	}
	...
	public pickTreeItemImpl(
		...
		for (const expectedContextValue of expectedContextValues) {
			...
			if (
				matchContextValue(expectedContextValue, [
					FunctionTreeItem.contextValue,
					FunctionsTreeItem.contextValue,
				])
			) {
			...
			const actionsContextValues = [
				ActionsTreeItem.contextValue,
				ActionTreeItem.contextValueCompleted,
