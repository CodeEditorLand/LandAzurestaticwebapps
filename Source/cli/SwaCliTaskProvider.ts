...
import {
	SWACLIOptions,
	tryGetStaticWebAppsCliConfig,
} from "./tryGetStaticWebAppsCliConfig";
...
export class SwaTaskProvider implements TaskProvider {
	...
	private async getTasksFromDetector(
		...
		appFolders.forEach((appFolder) => {
			...
			if (buildPreset) {
				tasks.push(
					this.createSwaCliTask(
						workspaceFolder,
						`start ${path.basename(appFolder.uri.fsPath)}`,
						{
							context: `http://localhost:${buildPreset.port}`,
							...(apiLocations?.length
								? { apiLocation: funcAddress }
								: {}),
