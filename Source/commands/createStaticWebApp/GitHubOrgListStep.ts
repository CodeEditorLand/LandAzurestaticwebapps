...
export class GitHubOrgListStep extends AzureWizardPromptStep<IStaticWebAppWizardContext> {
	public async prompt(context: IStaticWebAppWizardContext): Promise<void> {
		...
		do {
			orgData = (
				await context.ui.showQuickPick(this.getOrganizations(context), {
					placeHolder,
				})
			).data;
		} while (!orgData);
	...
	private async getOrganizations(
		...
		IAzureQuickPickItem<
			ListOrgsForUserData | OrgForAuthenticatedUserData | undefined
		>[]
	> {
		const octokitClient: Octokit = await createOctokitClient(context);

		const userData: OrgForAuthenticatedUserData =
			await GitHubOrgListStep.getAuthenticatedUser(
				context,
				octokitClient,
			);
