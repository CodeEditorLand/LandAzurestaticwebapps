...
export class BuildPresetListStep extends AzureWizardPromptStep<IStaticWebAppWizardContext> {
	public async prompt(context: IStaticWebAppWizardContext): Promise<void> {
		const placeHolder: string = localize(
			"choosePreset",
			"Choose build preset to configure default project structure",
		);

		const picks: IAzureQuickPickItem<IBuildPreset | undefined>[] =
			buildPresets.map((bp) => {
				return {
					...
					group:
						bp.group === "ssg"
							? localize("ssg", "Static site generator")
		...
		const pick: IAzureQuickPickItem<IBuildPreset | undefined> =
			await context.ui.showQuickPick(picks, {
				...
				learnMoreLink: "https://aka.ms/SWABuildPresets",
				enableGrouping: true,
			});
