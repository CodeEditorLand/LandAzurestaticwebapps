...
export class StaticWebAppNameStep extends AzureNameStep<IStaticWebAppWizardContext> {
	public async prompt(context: IStaticWebAppWizardContext): Promise<void> {
		const folderName: string = path.basename(
			nonNullProp(context, "uri").fsPath
		);
		...
		context.newStaticWebAppName = (
			await context.ui.showInputBox({
				...
				validateInput: async (
					...
					await this.validateStaticWebAppName(context, value),
			})
		).trim();
	...
	protected async isRelatedNameAvailable(
		...
		if (!context.newStaticWebAppName) {
			return await this.isSwaNameAvailable(
				context,
	...
	public async getRelatedName(
		...
		return await this.generateRelatedName(context, name, [
			staticWebAppNamingRules,
			resourceGroupNamingRules,
		]);
	}

	private async validateStaticWebAppName(
		context: IStaticWebAppWizardContext,
		...
		if (
			...
			return localize(
				...
				staticWebAppNamingRules.minLength,
				staticWebAppNamingRules.maxLength
			);
		} else if (staticWebAppNamingRules.invalidCharsRegExp.test(name)) {
			return localize(
				"invalidChars",
				'The name can only contain alphanumeric characters and the symbol "-"'
			);
		} else if (context.advancedCreation) {
			if (
				!(await this.isSwaNameAvailable(
					context,
					context.resourceGroup?.name,
					name
				))
				...
				return localize(
					"nameAlreadyExists",
					'Static web app name "{0}" already exists in your resource group "{1}".',
					name,
					context.resourceGroup?.name
				);
			}
	...
	private async isSwaNameAvailable(
		context: IStaticWebAppWizardContext,
