...
import {
	...
	GitWorkspaceState,
	remoteShortnameExists,
	VerifiedGitWorkspaceState,
	verifyGitWorkspaceForCreation,
	warnIfNotOnDefaultBranch,
} from "../../utils/gitUtils";
import { localize } from "../../utils/localize";
import { GitHubOrgListStep } from "./GitHubOrgListStep";
import { IStaticWebAppWizardContext } from "./IStaticWebAppWizardContext";

export async function setGitWorkspaceContexts(
	context: IActionContext & Partial<IStaticWebAppWizardContext>,
	folder: WorkspaceFolder
): Promise<void> {
	const gitWorkspaceState: GitWorkspaceState = await getGitWorkspaceState(
		context,
		folder.uri
	);
	...
	if (gitWorkspaceState.remoteRepo) {
		...
		if (!gitWorkspaceState.repo?.state.HEAD?.upstream) {
			...
			await context.ui.showWarningMessage(
				localize(
					"pushBranch",
					"Branch needs to exist on the remote. Push branch and set upstream?"
				),
				{
					modal: true,
				},
		...
		if (!context.advancedCreation) {
			...
			context.orgData =
				await GitHubOrgListStep.getAuthenticatedUser(context);
		}
	}
