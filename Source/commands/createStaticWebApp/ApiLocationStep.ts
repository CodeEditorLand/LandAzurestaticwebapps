...
export class ApiLocationStep extends AzureWizardPromptStep<IStaticWebAppWizardContext> {
	public async prompt(context: IStaticWebAppWizardContext): Promise<void> {
		const defaultValue: string =
			context.buildPreset?.apiLocation ?? defaultApiLocation;
		...
		context.apiLocation = context.detectedApiLocations?.length
			...
			: (
					await context.ui.showInputBox({
						...
						validateInput: (value) =>
							validateLocationYaml(value, "api_location"),
						learnMoreLink: "https://aka.ms/SwaApiLoc",
						prompt: localize(
							"enterApiLocation",
							"Enter the location of your Azure Functions code or leave blank to skip this step. For example, 'api' represents a folder called 'api'."
						),
					})
			  ).trim();
	...
	public shouldPrompt(context: IStaticWebAppWizardContext): boolean {
		if (context.detectedApiLocations?.length === 1) {
			...
			!context.detectedApiLocations?.length
		) {
			context.apiLocation =
				context.buildPreset?.apiLocation ?? defaultApiLocation;
		}
