 ...
 */
export async function tryGetApiLocations(
		...
		async () => {
			...
			if (await AzExtFsExtra.pathExists(folderPath)) {
				if (await isFunctionProject(folderPath)) {
					...
					const hostJsonUris = await workspace.findFiles(
						new RelativePattern(
							workspaceFolder,
							`*/${hostFileName}`
						)
...
export async function promptForApiFolder(
	context: IActionContext,
	...
	return (
		await context.ui.showQuickPick(
			detectedApiLocations.map((apiPaths) => ({ label: apiPaths })),
			{
				placeHolder: localize(
					"selectApi",
					"Select the location of your Azure Functions code"
				),
			}
		)
	).label;
...
async function isFunctionProject(folderPath: string): Promise<boolean> {
	return await AzExtFsExtra.pathExists(
		Utils.joinPath(URI.parse(folderPath), hostFileName)
	);
}
