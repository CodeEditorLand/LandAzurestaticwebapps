...
export async function postCreateStaticWebApp(
	swaNode: ResolvedStaticWebAppTreeItem
): Promise<void> {
	return await callWithTelemetryAndErrorHandling(
		"staticWebApps.postCreateStaticWebApp",
		async (context: IActionContext): Promise<void> => {
			const realSwaNode:
				| (ResolvedStaticWebAppTreeItem & AzExtParentTreeItem)
			...
			>(await realSwaNode.loadAllChildren(context)).find((ti) => {
				return (
					ti instanceof EnvironmentTreeItem &&
			...
			if (productionEnv) {
				...
				while (!deployActionNode) {
					try {
						...
						const filteredTreeItems: ActionTreeItem[] =
							...
							}); // only looking at on-going or queued jobs

						const promises: Promise<void>[] = filteredTreeItems.map(
							async (ti) => {
								const workflow: ActionsGetWorkflowResponseData =
									(
										await octokitClient.actions.getWorkflow(
											{
												owner,
												repo: name,
												workflow_id:
								...
								if (
									workflow.path.includes(
										swaNode.defaultHostname.split(".")[0]
									)
								) {
									deployActionNode = ti;
								}
					...
					if (Date.now() > maxTime) {
						throw new UserCancelledError(
							"postCreateStaticWebAppTimedOut"
						);
					}
				...
				const conclusion: Conclusion = await checkActionStatus(
					context,
					deployActionNode,
					true
				);
				...
				if (success || conclusion === Conclusion.Failure) {
					...
					const deploymentMsg: string = success
						? localize(
								"deploymentCompleted",
								'Successfully built and deployed "{0}". Commit and push changes to the GitHub repository to create a new deployment.',
								swaNode.name
						  )
						: localize(
								"deploymentFailed",
					...
					void window
						...
						.then(async (input) => {
							if (input === browseWebsite) {
								...
								await openUrl(
									nonNullValue(deployActionNode).data.html_url
								);
							}
						});
				}
