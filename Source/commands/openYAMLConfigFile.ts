...
import {
	BuildConfig,
	WorkflowGroupTreeItem,
} from "../tree/WorkflowGroupTreeItem";
import { localize } from "../utils/localize";

type YamlTreeItem =
	| ResolvedStaticWebApp
	| EnvironmentTreeItem
	| WorkflowGroupTreeItem;

export async function openYAMLConfigFile(
	...
	if (node instanceof WorkflowGroupTreeItem) {
		...
		node instanceof EnvironmentTreeItem &&
		node.gitHubConfigGroupTreeItems.length
	) {
		const picks: IAzureQuickPickItem<string>[] =
			node.gitHubConfigGroupTreeItems.map((configNode) => {
				return {
					label: basename(configNode.yamlFilePath),
					data: configNode.yamlFilePath,
				};
			});
		...
		const ymlFileName: string = `.github/workflows/azure-static-web-apps-${
			defaultHostname.split(".")[0]
		}.yml`;
...
export async function tryGetSelection(
	context: IActionContext,
	...
	try {
		...
		while (yamlNode) {
			if (
				"key" in yamlNode &&
				(<Scalar>yamlNode.key).value === buildConfigToSelect &&
				"value" in yamlNode
			) {
