...
function verifyTaskForkWorkspace(testCase: ITestCase, task: Task) {
	...
	if (testCase.hasDabConfig) {
		const message = `Expected --data-api-location argument to${
			testCase.hasDabConfig ? "" : " NOT"
		} be present in task shell execution. Found args: ${execution.args.join(
			" "
		)}`;
		assert.strictEqual(
			execution.args.includes("--data-api-location"),
			testCase.hasDabConfig,
			message
		);
	}
}
...
suite.only("SWA task provider", async () => {
	...
	for (const testCase of testCases) {
		test(testCase.workspaceName, async () => {
			...
			if (workspaceFolder) {
				const tasks =
					await taskProvider.provideTasksForWorkspaceFolder(
						workspaceFolder
					);
